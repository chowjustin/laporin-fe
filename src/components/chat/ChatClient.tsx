...
export default function ChatClient({
  ...
  const { sendMessage, lastMessage, readyState } = useWebSocket(WEBSOCKET_URL, {
    onOpen: () => {
      ...
      sendMessage(
        JSON.stringify({ type: "register", payload: { reportId, roles } })
      );
    },
    onClose: () => {
  ...
  // Fetch and decrypt historical messages
  useEffect(() => {
        ...
        try {
          ...
          const decryptedHistory = await Promise.all(
            historicalMessages.data.map(async (msg: any) => {
              ...
              const decryptKey =
                msg.sender === roles ? myKeys.publicKey : theirPublicKey;
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: "The final argument passed to useEffect changed size between renders. "
  useEffect(() => {
